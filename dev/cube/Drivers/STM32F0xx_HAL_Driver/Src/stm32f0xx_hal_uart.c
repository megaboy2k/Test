...
/*!
 * @brief   Enable receive interrupt to continuosly monitor RX line
 *          Use RxByteCallback to get each received byte
 * @param   huart
 * @return  HAL_StatusTypeDef
 */
HAL_StatusTypeDef HAL_UART_ReceiveEnable_IT( UART_HandleTypeDef *huart )
{
    /* Process Locked */
    __HAL_LOCK(huart);
...
#if (USE_HAL_UART_REGISTER_CALLBACKS == 1)
  if( huart->RxByteCallback != NULL )
  {
      huart->RxByteCallback( huart, (uint16_t)(huart->Instance->RDR & huart->Mask) );
  }
#endif
